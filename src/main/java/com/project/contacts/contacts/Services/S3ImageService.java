...
public class S3ImageService {
    ...
    public String upload(MultipartFile multiPartFile, String userId) throws Exception {

        try {

            final File file = convertMultiPartFileToFile(multiPartFile);
            final String fileName = userId + "_" + LocalDateTime.now();
            LOG.info("Uploading file with name {}", fileName);
            final PutObjectRequest putObjectRequest = new PutObjectRequest(s3BucketName, fileName + ".jpg", file);
            ...
            Files.delete(file.toPath()); // Remove the file locally created in the project folder
            LOG.info("Downloading file with name {}", startingUrl + fileName + ".jpg");
            return startingUrl + fileName + ".jpg";
        } catch (AmazonServiceException e) {
